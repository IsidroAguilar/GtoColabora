...
function site5framework_archive_nag(){
    ...
    if ( $pagenow == 'themes.php' ) {
		 ...
		 _e('Portfolio archive pages will only display in WordPress 3.1 or above.  Please upgrade.', 'gpp_theme');
		 echo '</p></div>';
    }
}
...
function wpthemess_content_nav() {
	...
	if (  $wp_query->max_num_pages > 1 ) :
		if (function_exists('wp_pagenavi') ) {
        	...
        	<nav id="nav-below">
			<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'gpp_theme' ); ?></h1>
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'gpp_theme' ) ); ?></div>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'gpp_theme' ) ); ?></div>
			</nav><!-- #nav-below -->
    	<?php }
	endif;
}
...
function site5framework_comments($comment, $args, $depth) {
  ...
  <li <?php comment_class(); ?>>
    <article id="comment-<?php comment_ID(); ?>">
      ...
      <em style="display:block;margin-left:40px"><?php _e('Your comment is awaiting moderation.','gpp_theme') ?></em>
      <?php endif; ?>
